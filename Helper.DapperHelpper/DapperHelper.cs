...
namespace Dapper
{
        ...
        where TConnection : IDbConnection, new()
    {
        ...
        public List<TableDetail> GetTables(string sql)
        {
            List<TableDetail> tables = new List<TableDetail>();
            using (var cnn = Cnn)
            {
                cnn.Open();
                DataTable dt = ((DbConnection)cnn).GetSchema("Tables");
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    string table_name = string.Empty;
                    string table_type = string.Empty;
                    string table_create = string.Empty;

                    try { table_name = dt.Rows[i]["table_name"]?.ToString(); } catch { }
                    try { table_type = dt.Rows[i]["table_type"]?.ToString(); } catch { }
                    try { table_create = dt.Rows[i]["date_created"]?.ToString(); } catch { }

                    tables.Add(new TableDetail()
                    {
                        TableName = table_name,
                        TableType = table_type,
                        DateCreated = table_create
                    });
                }
                cnn.Close();
            }
        ...
        public T QueryScalar<T>(string sql, object param = null, CommandType? commandType = null)
        {
            using (var cnn = Cnn)
                return cnn.ExecuteScalar<T>(sql, param, commandType: commandType);
        }
        ...
        {
            using (var cnn = Cnn)
                return cnn.QueryMultiple(sqls, param, commandType: commandType);
        }
